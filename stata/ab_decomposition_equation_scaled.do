...
tab survey mt_mom if bw60lag==0 [aweight = scaled_weight], row // validating this is the same as svy
...
tab survey firstbirth if bw60_mom==1 & bw60_mom[_n-1]==1 & SSUID==SSUID[_n-1] & PNUM==PNUM[_n-1] & year==(year[_n-1]+1) in 2/-1 [aweight = scaled_weight]
...
foreach var in mt_mom ft_partner_down_mom ft_partner_down_only ft_partner_leave lt_other_changes{
	tab `var' trans_bw60_alt2 if bw60lag==0 [aweight = scaled_weight], row
	tab `var' trans_bw60_alt2 if survey==1996 & bw60lag==0 [aweight=scaled_weight], row
	tab `var' trans_bw60_alt2 if survey==2014 & bw60lag==0 [aweight=scaled_weight], row
}
...
tab survey trans_bw60_alt2 if bw60lag==0 [aweight = scaled_weight], row
tab trans_bw60_alt2 if bw60lag==0 & survey==1996 [aweight = scaled_weight] // the numbers are different when just restricting to 1996 v. in full sample
tab trans_bw60_alt2 if bw60lag==0 & survey==2014 [aweight = scaled_weight]  

tab mt_mom survey if bw60lag==0 [aweight = scaled_weight], column
tab mt_mom if bw60lag==0 & survey == 2014 [aweight = scaled_weight]
...
putexcel set "$results/Breadwinner_Predictor_Equation_scaled", sheet(partner_ref) replace
...
tab survey ever_bw60 [aweight= scaled_weight], row
// do I want all instances even if mom like 2 years in the panel a BW?
tab survey bw60_mom [aweight= scaled_weight], row // okay this is lower, because ever_bw60 populated in all years.
